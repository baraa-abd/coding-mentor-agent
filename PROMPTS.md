...
When designing my prompts the main things I was concerned with were writing what the agent has to do in a precise and clear way (which was harder than I thought given the complications of the memory system). Beyond that, I was concerned with the order and scope of different portions (especially in the system prompt). I also used lots of examples in different prompts (one-shot or few-shot prompting). 


## Prompt with iteration:

Before (note that I was trying to store more in the topics memory at this point, but it's besides the point):
def UPDATE_TOPICS_PROMPT(
    scratchpad: dict,
    session_id: str,
) -> str:
    return f"""
    This session's scratchpad, which includes new topics with keys like "new_topic_<id>" and observations regarding old topics with keys like "user_level_<topic_id>", "note_<topic_id>", or "explanation_history_<topic_id>":
{scratchpad}

Session ID: {session_id}

For each new topic, output a complete JSON object including all fields.
For each old topic that was genuinely discussed (not simply inspected via some tool; base this on the scratchpad observations), output the updated mutable fields.

Rules for new topics:
1. id: snake_case unique id (make sure it doesn't collide with existing topics) WITHOUT the "new_topic_" prefix used in the scratchpad
2. title: human readable title
3. short_description: 1-3 sentence description of the topic
4. user_level: a numeric value between 0.0 and 1.0 (based on evidence in this session)
5. explanation_history: a list of strings, start with an entry for how you explained it this session (STRICT ≤20 words, focused on explanation type, not content) and include this session_id
6. note: any key observations from this session (STRICT ≤80 tokens)
7. prerequisites: list of topic_ids that are prerequisites. Must be exhaustive at creation time.
8. is_prerequisite_of: connections to existing topics (only added to when a new topic is created that depends on this one). 

Rules for old topics:
1. user_level (0.0–1.0)):
   - Only raise if there is behavioral evidence (correct application, exercise passed, code written correctly unprompted).
   - Only lower if the user demonstrably struggled with something they previously handled well.
   - If no clear evidence either way, leave unchanged.
   - "unknown" → numeric value based on available evidence. It should never be kept as "unknown" after the topic has been discussed in at least one session.
   - Updates based on observed behavior (e.g. exercises, code demos, user-written code) carry more weight than updates based on user statements, which in turn carry more weight than your explanations.

2. explanation_history (list of strings): each entry is a natural language statement of how you explained this topic this session (e.g. "simple exposition", "worked example", etc.), along with the session_id.
    - Append to existing history list.
    - If no new explanation, leave unchanged.
    - New entries should be concise (STRICT ≤20 words) and focused on the type of explanation, not the content (e.g. "conceptual explanation", "worked example with X code", "exercise with Y feedback").

3. note (string, STRICT ≤80 tokens): Key observations about the user's understanding of this topic.
   If adding new content would exceed 80 tokens, compress/summarize the existing note first.
   Focus on: recurring mistakes, preferred explanation styles, unresolved confusions.

4. is_prerequisite_of: Only update if new topics were discussed this session and connections were clearly established to this topic.



Output a JSON object mapping topic_id to its complete JSON object or updated mutable fields:
{{"topic_id": {{"title": "...", "short_description": "...", "user_level": ..., "explanation_history": [...], "note": "...", ...}}, ...}}
...
The change: reorganized to be more direct about what keys look like and what they correspond to. Rephrased evidence weight statement to be more explicit about the different kinds of evidence and their relative weights (and repeated it for good measure). Became explicit about what fields should be output. Changed the output format scheme since the old one just applied to new topics where all fields are updated, which could be confusing to the agent. 

## All prompts (copied from prompts.py):
